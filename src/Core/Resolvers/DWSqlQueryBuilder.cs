...
namespace Azure.DataApiBuilder.Core.Resolvers
{
    ...
    public class DwSqlQueryBuilder : BaseTSqlQueryBuilder, IQueryBuilder
    {
        ...
        private string BuildSqlQuery(SqlQueryStructure structure, string? subQuery)
        {
            ...
            {
                query.Append($" GROUP BY {string.Join(", ", structure.GroupByMetadata.Fields.Values.Select(c => Build(c)))}");
            }
            ...
            if (structure.GroupByMetadata.Aggregations.Count > 0)
            {
                ...
                {
                    query.Append($" HAVING {Build(havingPredicates)}");
                }
            }

            if (structure.OrderByColumns.Any())
            {
                query.Append($" ORDER BY {Build(structure.OrderByColumns)}");
            }

            query.Append($" OFFSET {structure.Offset()} ROWS FETCH NEXT {structure.Limit()} ROWS ONLY");
